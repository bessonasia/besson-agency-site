...
// === CUSTOM CURSOR SAFARI FIX: не даём родным стилям «включать» курсор
(() => {
  const ensureNone = () => {
    const d = document;
    // на всякий случай — прямые инлайновые стили на корне и body
    d.documentElement.style.cursor = 'none';
  ...
  // при каждом входе мыши / фокусе окна — повторяем
  window.addEventListener('mousemove', ensureNone, { passive: true });
  window.addEventListener('mouseenter', ensureNone, { passive: true });
  window.addEventListener('focus', ensureNone, { passive: true });

  // если кто-то динамически меняет class/style на <html> или <body> — возвращаем
  new MutationObserver(ensureNone).observe(document.documentElement, {
    attributes: true,
